    ...
    if(i == config.imageid):
        ...
        if(dominant_class == actual_label):
            ...
            if(dominant_class == actual_label):
                with open(lb_fullpath, 'a+', newline='') as write_obj:
                    csv_writer = csv.writer(write_obj)
                    csv_writer.writerow([net_file, str(dataset), "img "+str(i)+" with label "+str(actual_label), "eps="+str(epsilon), "DeepSRGR", str(end - start)+" secs", "success"])
                print("image ", i, " success!")
            else:
                with open(lb_fullpath, 'a+', newline='') as write_obj:
                    csv_writer = csv.writer(write_obj)
                    csv_writer.writerow([net_file, str(dataset), "img "+str(i)+" with label "+str(actual_label), "eps="+str(epsilon), "DeepSRGR", str(end - start)+" secs", "DK"])
                print("image ", i, " DK!")
